<template>
  <div id="app">
    <div class="container">
      ...
      <svg class="icone" @click="click()" version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 512 512" style="enable-background:new 0 0 512 512;" xml:space="preserve">
          <g>
              <g>
...
export default {
  ...
  methods:{
    obterInformacoes(){
      axios.get('https://viacep.com.br/ws/01001000/json/').then((resposta)=>{
        console.log(resposta.data);
        //alert(`A rua é: ${resposta.data.logradouro}`)
      })
    },
    click(){
      axios.get('https://viacep.com.br/ws/'+this.cepUsuario+'/json/').then((resposta)=>{
        if (resposta.data.localidade == 'Iporá') {
          console.log(resposta.data);
          alert(`Esse CEP pertence a Iporá`)
        } else {
          alert(`Esse CEP não pertence a Iporá`);
        }
      })
    }
  }
